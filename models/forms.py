...
class SeachForm(FlaskForm):
    tags = CustomSelectMultipleField("Tags", coerce=str, choices=[])
    submit = SubmitField("Search")

    def populate_tech_skills(self):
        self.tags.choices = [
            (str(skill.name),
             skill.name) for skill in storage.all(Tag).values()
        ]
...
class ProjectCreateForm(FlaskForm):
    name = StringField("Name",
                       validators=[DataRequired(),
                                   Length(min=2, max=20)])
    github_link = URLField("Github", validators=[DataRequired(), URL()])
    demo_link = URLField("Demo", validators=[Optional(), URL()])
    description = TextAreaField(
        "Description", validators=[DataRequired(), Length(min=2, max=2000)]
    )
    background_image = FileField(
        "System Design image", validators=[FileAllowed(["jpg", "png"])]
    )
    db_image = FileField("Database image",
                         validators=[FileAllowed(["jpg", "png"])])
    infra_image = FileField(
        "Infrastructure image", validators=[FileAllowed(["jpg", "png"])]
    )
